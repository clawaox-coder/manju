...
export default function Scenes() {
  ...
  const cats = useMemo(() => ['全部', ...new Set(scenes.map((s) => s.cat))], [scenes]);
        ...
        {filtered.map((s, i) => (
          ...
          >
            <Card className="overflow-hidden cursor-pointer hover:shadow-lg transition group" onClick={() => toast.success(`已应用「${s.name}」`)}>
              <div className={cn('aspect-video relative', s.bg)}>
                <Badge className="absolute top-2 right-2 backdrop-blur bg-black/40 text-white">{s.cat}</Badge>
              </div>
              <div className="p-3">
                <div className="font-semibold text-sm mb-1">{s.name}</div>
                <div className="flex items-center justify-between text-[11px] text-muted-foreground">
                  <span>已用 {s.uses} 次</span>
